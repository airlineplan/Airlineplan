...
const getPeriodSortKey = (dateStr, periodicity) => {
  ...
  switch (periodicity) {
    ...
    case "monthly": {
      ...
      return `${year}-${String(lastDay.getMonth() + 1).padStart(2, "0")}-${String(lastDay.getDate()).padStart(2, "0")}`;
    }
    case "quarterly": {
      const q = Math.ceil(month / 3);
      const lastMonthOfQ = q * 3;
      const lastDay = new Date(year, lastMonthOfQ, 0);
      return `${year}-${String(lastMonthOfQ).padStart(2, "0")}-${String(lastDay.getDate()).padStart(2, "0")}`;
    }
    case "weekly": {
    ...
    case "daily": return `${year}-${String(month).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
...
const MultiSelectDropdown = ({ placeholder, options = [], onChange, selected = [] }) => {
  ...
  return (
    <div className="relative w-full" ref={containerRef}>
      <button onClick={() => setIsOpen(!isOpen)} className={cn("w-full flex items-center justify-between px-3 py-2 text-sm rounded-lg border transition-all duration-200 bg-white dark:bg-slate-900/50", isOpen ? "border-indigo-500 ring-1 ring-indigo-500" : "border-slate-300 dark:border-slate-700 hover:border-slate-400")}>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 5 }} className="absolute z-[100] w-full mt-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl max-h-60 overflow-y-auto custom-scrollbar">
            {safeOptions.map((opt) => {
              const isSelected = selected.some(s => s.value === opt.value);
              return (
                <div key={opt.value} onClick={() => toggleOption(opt)} className="flex items-center px-3 py-2 text-sm cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                  <div className={cn("w-4 h-4 rounded border mr-3 flex items-center justify-center transition-colors shrink-0", isSelected ? "bg-indigo-500 border-indigo-500" : "border-slate-300 dark:border-slate-600")}>
...
const SingleSelectDropdown = ({ placeholder, options = [], onChange, selected }) => {
  ...
  const handleSelect = (opt) => { setIsOpen(false); if (onChange) onChange(opt); };
  const safeOptions = Array.isArray(options) ? options : [];

  return (
    <div className="relative w-full" ref={containerRef}>
      <button onClick={() => setIsOpen(!isOpen)} className={cn("w-full flex items-center justify-between px-3 py-2 text-sm rounded-lg border transition-all duration-200 bg-white dark:bg-slate-900/50", isOpen ? "border-indigo-500 ring-1 ring-indigo-500" : "border-slate-300 dark:border-slate-700 hover:border-slate-400")}>
        <span className="text-slate-700 dark:text-slate-200 font-medium truncate">{selected ? selected.label : placeholder}</span>
        <ChevronDown size={14} className={cn("text-slate-400 transition-transform ml-2", isOpen && "rotate-180")} />
      </button>
      <AnimatePresence>
        {isOpen && (
          <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 5 }} className="absolute z-[100] w-full mt-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl max-h-60 overflow-y-auto custom-scrollbar">
            {safeOptions.map((opt) => (
              <div key={opt.value} onClick={() => handleSelect(opt)} className="flex items-center px-4 py-2 text-sm cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors text-slate-700 dark:text-slate-300">
                {opt.label}
              </div>
            ))}
          </motion.div>
...
const PERIODICITY_OPTIONS = [
  { label: "Annually", value: "annually" }, { label: "Quarterly", value: "quarterly" },
  { label: "Monthly", value: "monthly" }, { label: "Weekly", value: "weekly" }, { label: "Daily", value: "daily" }
];

// FIXED: Exact match to database keys.
...
const GROUPING_OPTIONS = [
  ...
  { label: "Dep Stn", value: "depStn" }, { label: "Arr Stn", value: "arrStn" },
  { label: "Sector", value: "sector" }, { label: "Flight #", value: "flight" },
  { label: "Aircraft", value: "variant" }, { label: "Rotation #", value: "rotationNumber" },
...
const ListTable = () => {
  ...
  const [filters, setFilters] = useState({
    label: LABEL_OPTIONS[2], periodicity: PERIODICITY_OPTIONS[2], metric: METRIC_OPTIONS[0],
    from: [], to: [], sector: [], variant: [], userTag1: [], userTag2: [], rotation: [], aircraft: [],
    depTimeFrom: "", depTimeTo: "", arrTimeFrom: "", arrTimeTo: ""
  });

  const [level1, setLevel1] = useState(GROUPING_OPTIONS[4]);
  const [level2, setLevel2] = useState(GROUPING_OPTIONS[5]);
  const [level3, setLevel3] = useState(GROUPING_OPTIONS[0]);
      ...
      try {
        ...
        if (response.data) {
          setDropdownOptions(prev => ({
            ...prev,
            from: response.data.from || [], to: response.data.to || [], sector: response.data.sector || [],
            variant: response.data.variant || [], userTag1: response.data.userTag1 || [],
            userTag2: response.data.userTag2 || [], rotation: response.data.rotation || [], aircraft: response.data.aircraft || [],
          }));
        }
  ...
  useEffect(() => { fetchListData(); }, [filters]);
  ...
  const { tableColumns, tableData } = useMemo(() => {
    ...
    // 1️⃣ PREPROCESS FLIGHTS (Lightweight Math Extraction)
    const processedFlights = rawFlights.map(f => {
      ...
      if (metricKey === "departures") {
        ...
        val = Number(f[metricKey]) || 0; // DB ensures this is clean
      }
    ...
    const buildTree = (subset, depth) => {
      ...
      sortedKeys.forEach(key => {
        ...
        const groupTotals = Array(sortedColumns.length).fill(0);

        groupFlights.forEach(f => {
          const colIndex = columnIndexMap[f._periodKey];
          if (colIndex !== undefined) groupTotals[colIndex] += f._val;
        });

        finalRows.push({
          id: idCounter++, type: groupLabel, label: key, level: depth, data: groupTotals, isTotalRow: false
        });

        const hasChildren = buildTree(groupFlights, depth + 1);

        if (hasChildren || depth < hierarchyLevels.length - 1) {
          finalRows.push({ id: idCounter++, type: "Total", label: `${key} Total`, level: depth, data: groupTotals, isTotalRow: true });
        }
      });
      return true;
    };
    ...
    const decimalMetrics = ["fh", "bh"]; // Only these remain decimals
    const isDecimalMetric = decimalMetrics.includes(filters.metric.value);

    const grandTotals = Array(sortedColumns.length).fill(0);
    finalRows.forEach(row => {
        if (row.level === 0 && !row.isTotalRow) {
            row.data.forEach((val, idx) => grandTotals[idx] += val);
        }
    });

    const formattedRows = finalRows.map(row => ({
      ...row,
      data: row.data.map(val => isDecimalMetric ? Number(val.toFixed(2)) : Math.round(val))
    }));

    formattedRows.push({
        id: "grand-total", type: "Grand Total", label: "Grand Total", level: 0,
        data: grandTotals.map(val => isDecimalMetric ? Number(val.toFixed(2)) : Math.round(val)),
        isTotalRow: true, isGrandTotal: true
    });

    return { tableColumns: sortedColumns, tableData: formattedRows };
  ...
  const downloadExcel = () => {
    ...
    try {
      ...
      const excelRows = tableData.map((row) => {
        ...
        return [`${indent}${row.label}`, ...row.data];
      });
        ...
        <div className="flex-1 overflow-auto custom-scrollbar relative">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="bg-white/50 dark:bg-slate-900/50">
              {tableData.map((row) => (
                <tr key={row.id} className={cn("group transition-colors", row.isTotalRow ? "bg-slate-100 dark:bg-slate-800 font-semibold" : "hover:bg-indigo-50/50 dark:hover:bg-slate-800/30", row.isGrandTotal && "bg-emerald-50 dark:bg-emerald-900/20 border-t-2 border-emerald-500/30")}>
                  <td className={cn("sticky left-0 z-10 bg-white/95 dark:bg-slate-900/95 backdrop-blur border-r border-b border-slate-200 dark:border-slate-800 p-3 text-sm shadow-[4px_0_10px_-2px_rgba(0,0,0,0.05)]", row.isTotalRow ? "bg-slate-100/95 dark:bg-slate-800/95" : "group-hover:bg-indigo-50/90 dark:group-hover:bg-slate-800/90", row.isGrandTotal && "bg-emerald-50/95 dark:bg-emerald-900/95 text-emerald-700 dark:text-emerald-400 font-black", !row.isTotalRow && row.level === 0 && "font-bold text-slate-800 dark:text-slate-100 bg-slate-50/50", !row.isTotalRow && row.level === 1 && "pl-8 font-semibold text-slate-700", !row.isTotalRow && row.level === 2 && "pl-14 font-medium text-slate-600", row.isTotalRow && !row.isGrandTotal && "pl-4 italic")}>
                    <div className="flex items-center gap-2">
                  ...
                  {row.data.map((val, idx) => (
                    <td key={idx} className={cn("p-3 border-r border-b border-slate-200 dark:border-slate-800 text-center text-sm tabular-nums", row.isTotalRow ? "font-bold text-slate-800 dark:text-slate-200" : "text-slate-600 dark:text-slate-400", row.isGrandTotal && "text-emerald-700 dark:text-emerald-400 font-black")}>
                      {val.toLocaleString()}
                    </td>
                  ))}
                </tr>
