...
const Rotations = () => {
    ...
    const loadDropdowns = async () => {
      try {
        ...
        const [varRes, rotRes] = await Promise.all([
          axios.get("http://localhost:3000/listVariants", { headers: { "x-access-token": token } }),
          axios.get("http://localhost:3000/listRotations", { headers: { "x-access-token": token } })
        ]);
  ...
  const getFgtsWORotations = async () => {
    ...
    try {
      const res = await axios.post("http://localhost:3000/flightsWoRotations", requestData, {
  ...
  const handleRotationChange = async (val) => {
    if (val === "new") {
      try {
        const res = await axios.get("http://localhost:3000/getNextRotationNumber", {
          headers: { "x-access-token": localStorage.getItem("accessToken") }
        });
      ...
      try {
        const res = await axios.get(`http://localhost:3000/rotationbyid/${val}`, {
          headers: { "x-access-token": localStorage.getItem("accessToken") }
        });
  ...
  const handleSave = async () => {
    ...
    try {
      await axios.post("http://localhost:3000/updateRotationSummary", {
  ...
  const handleAddCurrent = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await axios.post("http://localhost:3000/addRotationDetailsFlgtChange", {
        rotationNumber: selectedRotation,
  ...
  const handleDeleteRotation = async () => {
    ...
    try {
      await axios.post("http://localhost:3000/deleteCompleteRotation", {
        rotationNumber: selectedRotation,
        selectedVariant,
  ...
  const handlePreviousInRotation = async () => {
    ...
    try {
        ...
        if(!lastObject) {
             res = await axios.post("http://localhost:3000/deleteRotation", {
            ...
            res = await axios.post("http://localhost:3000/deletePrevInRotation", {
                rotationNumber: selectedRotation, selectedVariant,
      ...
      <div className="flex-1 flex flex-col gap-6 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col bg-white/70 dark:bg-slate-900/60 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-2xl shadow-lg overflow-hidden min-h-[300px]">
            <div className="px-5 py-3 border-b border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50">
                <h3 className="font-bold text-slate-700 dark:text-slate-200 flex items-center gap-2">
                    <Search className="text-blue-500" size={18} /> Flights not assigned to a rotation
                </h3>
            </div>
