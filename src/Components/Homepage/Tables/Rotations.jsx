...
import { Container, Stack, Box, Grid, Paper, Table, TableHead, TableRow, IconButton, TableCell, TableBody, TextField, Button, MenuItem, Typography, Pagination } from '@mui/material';
...
const RotationDropDownMenu = ({ data, handleRotationChange, selectedRotation }) => {
    ...
    const isRotationValInOptions = options.some((item) => item.value === newRotationVal);
...
const Rotations = () => {
    ...
    const handleRotationChange = async (event) => {
        if (event == "new") {
            try {
                ...
                const response = await axios.get("https://airlineplan.com/getNextRotationNumber", {
                    headers: {
            ...
            try {
                const accessToken = localStorage.getItem("accessToken");
                const response = await axios.get(`https://airlineplan.com/rotationbyid/${event}`, {
                    headers: {
                        "x-access-token": accessToken,
                    },
                });
    ...
    }, [selectedRotation, selectedVariant, effFromDate, effToDate, rotationDevelopmentTableData, editable]);
    ...
    const getFgtsWORotations = async () => {
        ...
        const responseFlgts = await axios.post('https://airlineplan.com/flightsWoRotations', requestData, {
            headers: {
    ...
    const handleAddCurrent = async () => {
        try {
            ...
            const response = await axios.post('https://airlineplan.com/addRotationDetailsFlgtChange', {
                rotationNumber: selectedRotation,
    ...
    const handleDeleteRotation = async () => {
        try {
            ...
            const response = await axios.post('https://airlineplan.com/deleteCompleteRotation', {
                rotationNumber: selectedRotation,
            ...
            if (response.status === 201 || response.status === 200) {
                ...
                setRotationDevelopmentTableData(prevData => {
                    return prevData.filter(row => row.rotationNumber !== selectedRotation && row.variant !== selectedVariant);
                });
    ...
    const handlePreviousInRotation = async () => {
        try {
            ...
            if (!lastObject) {
                response = await axios.post('https://airlineplan.com/deleteRotation', {
                    rotationNumber: selectedRotation,
                    selectedVariant: selectedVariant,
                ...
                response = await axios.post('https://airlineplan.com/deletePrevInRotation', {
                    rotationNumber: selectedRotation,
                    selectedVariant: selectedVariant,
    ...
    const addTime = (firstTime, secondTime) => {
        ...
        const totalMinutes = parseInt(firstTime.split(':')[0]) * 60 + parseInt(firstTime.split(':')[1]) +
            parseInt(secondTime.split(':')[0]) * 60 + parseInt(secondTime.split(':')[1]);
    ...
    useEffect(() => {
        (async () => {
            try {
                const accessToken = localStorage.getItem("accessToken");
                const response = await axios.get("https://airlineplan.com/listVariants", {
                    headers: {
                        "x-access-token": accessToken,
                    },
                });
        ...
        <>
            ...
            <div>
                <Grid container mt={3} spacing={2} >
                        ...
                        <Paper  >
                            <Stack direction="column" spacing={1}>
                                <Stack direction="row" justifyContent="space-between" alignItems="center">
                                    ...
                                    <Typography color="text.secondary" style={{ fontSize: '14px' }}>Variant:</Typography>
                                    <DropDownMenu data={listOfVariant} onChange={handleDropDownChange} fieldProps={fieldProps} selectedValue={selectedVariant} />

                                </Stack>
                                <Stack direction="row" justifyContent="space-between" alignItems="center">
                                    {/* Labels and Dropdowns */}
                                    <Typography color="text.secondary" style={{ fontSize: '14px' }}>Rotation #:</Typography>
                                    <RotationDropDownMenu data={listOfRotation} handleRotationChange={handleRotationChange} selectedRotation={selectedRotation} />
                                </Stack>
                                {/* Add more rows as needed */}
                                <Stack direction="row" justifyContent="space-between" alignItems="center">
                                    <Typography color="text.secondary" style={{ fontSize: '14px' }}>Rotation Tag:</Typography>
                                    <TextField
                                        {...fieldProps}
                                        size="small"
                                        style={{ width: 135 }}
                                ...
                                <Stack direction="row" justifyContent="space-between" alignItems="center">
                                    <Typography color="text.secondary" style={{ fontSize: '14px' }}>Eff from date:</Typography>
                                    <div style={{ position: "relative", display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>
                                        ...
                                        <LocalizationProvider dateAdapter={AdapterDayjs}>
                                            ...
                                            {leftPanelError && !effFromDate && (
                                                <Typography variant="body2" sx={{ position: 'absolute', right: "100%", color: 'red' }}>
                                                    required
                                                </Typography>
                                            )}
                                ...
                                <Stack direction="row" justifyContent="space-between" alignItems="center">
                                    <Typography color="text.secondary" style={{ fontSize: '14px' }}>Eff to date:</Typography>
                                ...
                                <Stack direction="row" justifyContent="space-between" alignItems="center" >
                                    <Typography color="text.secondary" style={{ fontSize: '14px' }}>DoW:</Typography>
                                    <div style={{ position: "relative", display: 'flex', alignItems: 'bottom', justifyContent: 'flex-end' }}>
                                        <TextField
                                            {...fieldProps}
                                            value={dow}
                                            style={{ width: 135 }}
                                ...
                                <Stack direction="row" justifyContent="space-between" alignItems="center" >
                                    <Typography color="text.secondary" style={{ fontSize: '14px', flex: '1', textAlign: 'start' }}>BH total:</Typography>
                                    <Typography variant='p' color="text.secondary" style={{ flex: '1', textAlign: 'center' }}>{totalBt}</Typography>
                                </Stack>
                                <Stack direction="row" justifyContent="space-between" alignItems="center">
                                    <Typography color="text.secondary" style={{ fontSize: '14px', flex: '1', textAlign: 'start' }}>GT total:</Typography>
                                    <Typography variant='p' color="text.secondary" style={{ flex: '1', textAlign: 'center' }}>{totalGt}</Typography>
                                </Stack>
                                <Stack direction="row" justifyContent="space-between" alignItems="center">
                                    <Typography color="text.secondary" style={{ fontSize: '14px', flex: '1', textAlign: 'start' }}>Rtn total time:</Typography>
                                    <Typography variant='p' color="text.secondary" style={{ flex: '1', textAlign: 'center' }}>{addTime(totalBt, totalGt)}</Typography>
                                </Stack>
                                <Stack direction="row" justifyContent="end" sx={{ width: '100%', height: '32px', marginTop: '5px' }}>
                                    {/* Your other components here */}
                                    {editable && (
                                        <Box sx={{ mr: 2 }}> {/* Adjust the marginRight value as needed */}
                                            <Button variant="contained" color="primary" sx={{ width: '100px', height: '32px' }} onClick={handleSave}>
                                                Save
                                            </Button>
                                        </Box>
                                    )}
                                    ...
                                    {selectedRotation !== '' && (
                                        <TableRow key={1} style={{ width: "50px" }}>
                                            <TableCell sx={{
                                                ...
                                                fontSize: "12px",
                                                textAlign: "center",
                                            }}>{rotationDevelopmentTableData.length + 1}</TableCell>
                                            <TableCell sx={{
                                                padding: "8px",
                                                fontSize: "12px",
                                                textAlign: "center",
                                                ...
                                                <TextField
                                                    ...
                                                    inputProps={{ style: { padding: "10px", height: "15px", fontSize: "12px" } }}
                    ...
                    <Grid item xs={8} ml={4}>
                        <Typography color="text.secondary" style={{ fontSize: '16px' }}>Flights not assigned to a rotation</Typography>
