...
const Rotations = () => {
    ...
    const loadDropdowns = async () => {
      try {
        const [varRes, rotRes] = await Promise.all([
          api.get("/listVariants"),
          api.get("/listRotations")
        ]);
  ...
  const handleRotationChange = async (val) => {
    if (val === "new") {
      try {
        const res = await api.get("/getNextRotationNumber");
      ...
      try {
        const res = await api.get(`/rotationbyid/${val}`);
  ...
  const handleSave = async () => {
    ...
    try {
      await api.post("/updateRotationSummary", {
  ...
  const handleAddCurrent = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await api.post("/addRotationDetailsFlgtChange", {
        rotationNumber: selectedRotation,
  ...
  const handleDeleteRotation = async () => {
    ...
    try {
      await api.post("/deleteCompleteRotation", {
        rotationNumber: selectedRotation,
        selectedVariant,
